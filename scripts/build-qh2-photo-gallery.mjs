...
const defaultPublicBase =
	...
	process.env.VITE_PHOTO_GALLERY_PUBLIC_BASE ??
	...
	process.env.VITE_PHOTO_GALLERY_MANIFEST_URL?.replace(/\/manifest\.json$/i, "") ??
	"";

function parseArgs(argv) {
	const args = {
		input: "",
		output: "dist/qh2-photos/qh2/photos",
		publicBase: defaultPublicBase,
		fullPrefix: "originals",
		thumbPrefix: "thumbs",
	};
...
function printHelp() {
	console.log(`
Usage:
  node scripts/build-qh2-photo-gallery.mjs --input ./photos/originals

Options:
  --input         Local folder of original photos. Required.
  --output        Local output folder. Default: dist/qh2-photos/qh2/photos
  --public-base   Public base URL. Defaults to PHOTO_GALLERY_PUBLIC_BASE, VITE_PHOTO_GALLERY_PUBLIC_BASE, or manifest URL env without /manifest.json.
  --full-prefix   Output prefix for originals. Default: originals
